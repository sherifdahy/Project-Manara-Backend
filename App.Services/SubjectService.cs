...
public class SubjectService(UserManager<ApplicationUser> userManager,IUnitOfWork unitOfWork) : ISubjectService
{
    ...
    public async Task<bool> IsUserHasAccessToSubject(ClaimsPrincipal user, int requestSubjectId)
    {
        ...
        if (universityUser != null)
        {
            ...
            return await _unitOfWork.Subjects
                .IsExistAsync(f => f.Faculty.UniversityId == universityUser.UniversityId && f.Id == requestSubjectId);
        }
        ...
        if (facultyUser != null)
            return await _unitOfWork.Subjects.IsExistAsync(f => f.FacultyId == facultyUser.FacultyId && f.Id == requestSubjectId);
