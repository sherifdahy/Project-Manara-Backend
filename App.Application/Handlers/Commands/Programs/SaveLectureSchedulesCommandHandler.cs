...
public class SaveLectureSchedulesCommandHandler(
    ...
    DayErrors dayErrors) : IRequestHandler<SaveLectureSchedulesCommand, Result>
{
    ...
    public async Task<Result> Handle(SaveLectureSchedulesCommand request, CancellationToken cancellationToken)
    {
        #region Check Doctors
        ...
        var existingSchedules = await _unitOfWork.LectureSchedules.FindAllAsync(x => x.ProgramId == request.ProgramId && x.YearId == yearTerm.YearId && x.TermId == yearTerm.TermId, cancellationToken);

        if (existingSchedules.Any())
            _unitOfWork.LectureSchedules.DeleteRange(existingSchedules);
        #endregion
        ...
        if (request.Schedules.Any())
        {
            ...
            {
                var lectureSchedule = x.Adapt<LectureSchedule>();

                lectureSchedule.ProgramId = request.ProgramId;
                lectureSchedule.YearId = yearTerm.YearId;
                lectureSchedule.TermId = yearTerm.TermId;

                return lectureSchedule;
            }).ToList();

            await _unitOfWork.LectureSchedules.AddRangeAsync(newSchedules, cancellationToken);
        }
        #endregion
