...
public class GetLectureSchedulesQueryHandler(IUnitOfWork unitOfWork, ProgramErrors programErrors,YearErrors yearErrors) : IRequestHandler<GetLectureSchedulesQuery, Result<List<LectureScheduleItemResponse>>>
{
    ...
    public async Task<Result<List<LectureScheduleItemResponse>>> Handle(GetLectureSchedulesQuery request, CancellationToken cancellationToken)
    {
        ...
        var existingSchedules = await _unitOfWork.LectureSchedules.FindAllAsync(x => x.ProgramId == request.ProgramId && (x.YearId == yearTerm.YearId && x.TermId == yearTerm.TermId) && x.Subject.IsDeleted != true && x.Period.IsDeleted != true, x => x.Include(d => d.Subject), cancellationToken);
